...
//    	InputStream in = System.in;// = new System.in(args[0]);
//        int N = in.readInt();
    	Random r = new Random();
        int N = r.nextInt(100);
...
//                weight[i][j] = in.readDouble();
        ...
        for (int i = 0; i < N; i++)
            System.out.println(i + "-" + assignment.sol(i) + "' " + weight[i][assignment.sol(i)]);

        for (int i = 0; i < N; i++)
            System.out.println("px[" + i + "] = " + assignment.dualRow(i));
        for (int j = 0; j < N; j++)
            System.out.println("py[" + j + "] = " + assignment.dualCol(j));
        for (int i = 0; i < N; i++)
            for (int j = 0; j < N; j++)
                System.out.println("reduced[" + i + "-" + j + "] = " + assignment.reduced(i, j));
