...
//    double xnorm;
    ...
    gtol = 0.9;
    stpmin = pow(double(10), double(-20));//change this if you want a minimum step size (think that this is multiple by the gradient so x^(k+1)=x^k-step*gradient)
    stpmax = pow(double(10), double(20));//change this if you want a maximum step size (think that this is multiple by the gradient so x^(k+1)=x^k-step*gradient)
    ispt = n+2*m;
    iypt = ispt+n*m;
    for(i = 1; i <= n; i++)
    {
    ...
    stp1 = 10/gnorm;//change this if you want a different initial step size fo optimization
    ftol = 0.0001;
    maxfev = 20;
    while(true)
    {
