...
//=============================amatf:for unix compatibility=======================
#if !(defined(_WIN32) || defined(_WIN64))
void GetFileNameExtension(const char* f,char *name,char* ext)
{
	string FileName(basename((char*)f));//for some reason it does not compile with char* basename(const char* basename)
    if(FileName.find_last_of(".") != std::string::npos)
	{
		string extS=FileName.substr(FileName.find_last_of("."));
		ext[extS.size()]=0;//null terminated
		memcpy(ext,extS.c_str(),extS.size());
		
		extS=FileName.substr(0,FileName.find_last_of("."));
		name[extS.size()]=0;
		memcpy(name,extS.c_str(),extS.size());
	}
	else {
		ext[0]=0;
...
void SLIC::allocateLABvecvec()
{
	clearLABvecvec();
	int sz=m_height*m_width;
	
	m_lvecvec = new double*[m_depth];
	m_avecvec = new double*[m_depth];
	m_bvecvec = new double*[m_depth];
	for( int d = 0; d < m_depth; d++ )
	{
		m_lvecvec[d] = new double[sz];
		m_avecvec[d] = new double[sz];
		m_bvecvec[d] = new double[sz];
		memset(m_lvecvec[d],0,sz*sizeof(double));
		memset(m_avecvec[d],0,sz*sizeof(double));
		memset(m_bvecvec[d],0,sz*sizeof(double));
	}
}

void SLIC::clearLABvec()
{
	if(m_lvecvec)
	{
		delete [] m_lvec;
...
void SLIC::allocateLABvec()
{
	...
	int sz=m_height*m_width;
		...
		memset(m_lvec,0,sz*sizeof(double));
		memset(m_avec,0,sz*sizeof(double));
		memset(m_bvec,0,sz*sizeof(double));
}
...
//===========================================================================
///	DoRGBtoLABConversion: amatf
///

//from a single channel image (so r=g=b) with type imageType to LAB 
/// For whole volume
//===========================================================================
void SLIC::DoRGBtoLABConversion(
								...
								double**&					bvec)
{
	...
	for( int d = 0; d < m_depth; d++ )
	{
		for( int j = 0; j < sz; j++ )
		{
			...
			RGB2LAB( r, r, r, lvec[d][j], avec[d][j], bvec[d][j] );
		}
	}
}
...
//from a single channel image (so r=g=b) with type imageType to LAB 
/// For whole volume read with Mylib
//===========================================================================
void SLIC::DoRGBtoLABConversion(
								const imageType*&		ubuff,
								double**&					lvec,
								...
								double*&					bvec)
{
	...
	long int pos=0;
		...
		for( int j = 0; j < sz; j++ )
		{
			...
			RGB2LAB( r, r, r, lvec[j], avec[j], bvec[j] );
		}
...
///amatf: modified to incorporate Z anisotropy
//===========================================================================
	...
	const unsigned char 		maskThr)
{
	...
	int xstrips = (0.5+double(m_width)/double(STEP));
	int ystrips = (0.5+double(m_height)/double(STEP));
	int zstrips = (0.5+double(m_depth)/double(STEPZ));

	int xerr = m_width  - STEP*xstrips;
	int yerr = m_height - STEP*ystrips;
	int zerr = m_depth  - STEPZ*zstrips;
	...
	if(imgBckFwdMaskPtr==NULL)//classical approach without foreground/backgroudn mask
	{

		for( int z = 0; z < zstrips; z++ )
		{
			int ze = z*zerrperstrip;
			int d = (z*STEPZ+zoff+ze);
			for( int y = 0; y < ystrips; y++ )
			{
				int ye = y*yerrperstrip;
				for( int x = 0; x < xstrips; x++ )
	...
	}else{//new approach with foreground/background mask
		long int posOffset=0;
		for( int z = 0; z < zstrips; z++ )
		{
			int ze = z*zerrperstrip;
			int d = (z*STEPZ+zoff+ze);
			posOffset=d*m_width*m_height;
			for( int y = 0; y < ystrips; y++ )
			{
				int ye = y*yerrperstrip;
				for( int x = 0; x < xstrips; x++ )
				{
					...
					if(imgBckFwdMaskPtr[posOffset+i]>maskThr)//add seeds only in the fore ground
					{
						kseedsl[n] = m_lvecvec[d][i];
						kseedsa[n] = m_avecvec[d][i];
						kseedsb[n] = m_bvecvec[d][i];
...
///amatf: modification to incorporate Z anisotropy into method
//===========================================================================
	...
	const double&				anisotropyZ)
{
	...
	//amatf: it should be for 3D
	double invwt = 1.0/((STEP/m)*(STEP/m)*(STEPZ/m));

	double auxAnisotropy=anisotropyZ*anisotropyZ;
	...
	for( int itr = 0; itr < 5; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
			//amatf: this is the key for speed-> the search for candidate seeds is over a very limited range
			y1 = max(0.0,			kseedsy[n]-offset);
			y2 = min((double)m_height,	kseedsy[n]+offset);
			x1 = max(0.0,			kseedsx[n]-offset);
			x2 = min((double)m_width,	kseedsx[n]+offset);
			z1 = max(0.0,			kseedsz[n]-offsetZ);
			z2 = min((double)m_depth,	kseedsz[n]+offsetZ);
				...
				for( int y = y1; y < y2; y++ )
				{
					for( int x = x1; x < x2; x++ )
					{
						...
						distxyz =		(x - kseedsx[n])*(x - kseedsx[n]) +
										(y - kseedsy[n])*(y - kseedsy[n]) +
										(z - kseedsz[n])*(z - kseedsz[n])*auxAnisotropy; //amatf: scale distance in Z
						//------------------------------------------------------------------------
		...
		for( int d = 0; d < m_depth; d++  )
		{
			...
			for( int r = 0; r < m_height; r++ )
			{
				for( int c = 0; c < m_width; c++ )
				{
					auxIdx=klabels[d][ind];
					if(auxIdx>=0)//amatf: when we use foreground/background mask some pixels might not be assigned, so we need to introduce an if inside a for loop
					{
						sigmal[auxIdx] += m_lvecvec[d][ind];
						sigmaa[auxIdx] += m_avecvec[d][ind];
						sigmab[auxIdx] += m_bvecvec[d][ind];
						sigmax[auxIdx] += c;
						sigmay[auxIdx] += r;
						sigmaz[auxIdx] += d;
					
						clustersize[auxIdx] += 1.0;
					}
					ind++;
				}
			}
...
#else
	GetFileNameExtension(filename.c_str(),fname,extn);	
#endif
	string temp = fname;
	 ...
	 string&				path)
{
	...
	if(outfile.is_open())
		cout<<"SLIC::SaveSupervoxelLabels: writing labels at "<<finalpath<<endl;
	else
	{
		cout<<"ERROR: SLIC::SaveSupervoxelLabels file "<<finalpath<<" could not be opened"<<endl;
		exit(2);
	}
	for( int d = 0; d < depth; d++ )
	{
...
///amatf: here I do not need anisotropyZ because it is just an ad-hoc heuristic. We will look at a slightly larger box than needed in Z but it is Ok
//===========================================================================
...
//===========================================================================
///	amatf DoSuperpixelSegmentation_ForGivenStepSize
///
/// There is option to save the labels if needed.
//===========================================================================
void SLIC::DoSuperpixelSegmentation_ForGivenStepSize(
	...
	const double&				m)
{
	...
	allocateLABvec();//it frees memory if necessary
...
//===========================================================================
//amatf: adapted to accept our kinds of images and format
///	DoSupervoxelSegmentation
///
/// There is option to save the labels if needed.
//===========================================================================
									...
									const double&				anisotropyZ)
{
	...
	//--------------------------------------------------
	//reset values for class SLIC
	m_width  = width;
	m_height = height;
	m_depth  = depth;
	
	allocateLABvecvec();//it frees memory if necessary
	
	//--------------------------------------------------
	DoRGBtoLABConversion(ubuffvec, m_lvecvec, m_avecvec, m_bvecvec);
	//--------------------------------------------------
	
	GetKValues_LABXYZ(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz, STEP,anisotropyZ);
	
	PerformSupervoxelSLIC(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz, klabels, STEP, m, anisotropyZ);
	numlabels = kseedsl.size();
...
//===========================================================================
void SLIC::DoSupervoxelSegmentationWithMask(
									const imageType*		ubuffvec,
									const int&					width,
									const int&					height,
									const int&					depth,
									...
									const unsigned char 		maskThr)
{
	...
	GetKValues_LABXYZ(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz, STEP,anisotropyZ,imgBckFwdMaskPtr,maskThr);

	PerformSupervoxelSLIC(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz, klabels, STEP, m, anisotropyZ);
	numlabels = kseedsl.size();

	if(imgBckFwdMaskPtr==NULL)
		EnforceLabelConnectivity_supervoxels(width, height, depth, klabels, numlabels, STEP);
	else{
		//TODO: just recolor super-pixels with flood fill (like we do in opticalFlow code)
	}
